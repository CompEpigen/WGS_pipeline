...
for record in reader:
    ...
    cosmic = record.ID
    if (not (gene_name in genes_CH) or pop_freq>1e-3) and AC>2 and pop_freq>2e-5 and (pop_freq>1e-4 or len(cosmic)==0) and tcga_impact=="": filters.append("germline")
    #if len(cosmic)==0: filters.append("not-in-COSMIC")
    if description in ["intronic","5' UTR","3' UTR","non-coding-transcript","incomplete","silent","intergenic"]\
    ...
    #results["TCGA"].append(tcga_impact)
    ...
    if len(filters)==0:
        ...
        #results_filtered["TCGA"].append(tcga_impact)
